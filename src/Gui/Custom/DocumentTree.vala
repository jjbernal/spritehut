...
//                    print("Selected row: %s visible: %s\n", el.name, el.visible.to_string());
                    if (el is Layer) {
                        window.document.active_layer = (Layer) el;
                        
                    }
                    else if (el is SpriteHut.Data.Frame) {
                        window.document.active_frame = (SpriteHut.Data.Frame) el;
                    }
                    else if (el is Animation) {
                        window.document.active_animation = (Animation) el;
                        window.document.active_animation_iter = iter;
                    }
                    else if (el is Sprite) {
                        window.document.active_sprite = (Sprite) el;
                    }
